...
## 2026-09-13：未綁卡亦可預覽 Facebook 工作

使用者明確要求未綁卡師傅亦可看到 Facebook 工作預覽，綁卡後才可查看詳情。
此要求取代下文列表與詳情使用完全相同訂閱門檻的限制。DEV／PROD 的
`setup_required` 師傅可在統一列表預覽標題、地區及工種；伺服器只回傳預覽
欄位，遮蔽標題／地區內的直接聯絡資料，不回傳正文、聯絡資料或來源連結。
詳情及原帖入口仍須有效訂閱和兩項新工作權限，直接網址亦不可繞過。
原有香港／需求／hash／保存期限檢查、欠費暫停及終止規則維持不變。
...
## 2026-09-13：DEV 放寬香港工作線索門檻

使用者要求降低篩選門檻，重點收集香港「搵人開工」帖文。DEV 逐帖分析時，
明確搵師傅、請工人、找工程判頭合作、安裝／維修或工程報價需求均可納入；
毋須已有完整地址、工期、薪酬或直接電話。香港來源群組的內容與語境可作
香港線索依據，沒有明確海外工作地點時，不再只因缺少地區而隱藏。

此要求取代上文「僅香港群組名稱不足以確認、地區不明一律隱藏」的 DEV
篩選限制。來源語境推定香港須在 `intentReview.regionEvidence` 記錄
`source_context`，維持待核實；缺少地區時，`displayLocation` 顯示
「香港（地區未提供）」而不猜測地區。原有地名可保留，但有歧義的地名
不視為已核實在港；原文明確支持香港地點才記錄 `post_text`。仍須逐帖確認工作需求，
不得把所有香港群組帖文自動設為工作。

明確海外工作、服務／產品廣告、師傅自行求職、純 DIY 討論、已確認截止
或取消的需求仍排除。日期未知維持未知；不得用擷取時間聲稱新帖。
聯絡資料遮蔽、原文 hash、來源連結、去重、刪除／保存狀態、師傅訂閱
權限及禁止建立正式工作單的規則繼續生效。這次只更新 DEV 資料，
不自動同步或更改 PROD 紀錄。

## 2026-09-13：授權每日檢視及 DEV → PROD 同步

使用者其後要求把目前合資格資料同步到 DEV 及 PROD，並每日執行同一工作。
此授權取代上文只限 DEV、只限六個快照及一次性執行的限制，將放寬後的香港
需求篩選套用至每日檢視及兩個環境。未知日期仍維持未知，不設定固定日數窗口。

來源以 `tools/facebook_group_leads/sources.json` 的啟用清單為準：目前 24 個
已檢視的香港來源；曾見台灣為主內容的另一組停用。每組最多檢視八個可見帖，
可展開候選及核對原帖；遇登入失效、checkpoint、驗證碼、封鎖或權限問題停止。
本次不授權額外加入群組、擴大來源或任何對外聯絡。

每天先逐帖分析、遮蔽新候選及去重，先更新 DEV、驗證後再補同步 PROD。
只新增合資格外部快照，保留既有紀錄、來源、hash、待人工核實及保存／刪除狀態；
不恢復已刪除／到期來源，不同步原生客戶工作或帳戶，不建立正式工作單。
詳見 `tools/facebook_group_leads/README.md`；已授權的同步不再另行逐次要求確認。

## 2026-09-13：首頁真實工作摘要

使用者要求未登入首頁走馬燈改用真實資料。首頁可讀取目前環境最多 12 筆
合資格快照的簡短標題、概括地區及工種；Mongo projection 不讀取原文、
聯絡資料或來源連結。標題遮蔽直接聯絡資料，地區限已知香港區域名稱，
不顯示詳細地址。保留現有意圖、香港、review hash、刪除及到期篩選。
首頁統一說明「工作詳情及接洽安排，以發帖人確認為準」，不逐卡顯示
內部待核實標籤；內部核實狀態不變。不將擷取時間當作發帖日期，不保證仍有空缺；無資料
顯示空狀態。這是公開摘要的有限例外，詳情及聯絡入口仍須有效師傅訂閱，
不建立正式工作或對外聯絡。使用者其後已批准部署至 DEV 及 PROD；只部署網站改動，不同步資料。
